...
jQuery.fn.qucikPage = function(options) {
	...
	if (options.currentpage == 1 || paggerCounter == 1) {
	...
	function previous() {
		...
		if (options.currentpage > 1) {
			options.currentpage--;
			element.children().hide();
			element.children(".page" + options.currentpage).show(200);
	...
	function updateview() {
		...
		$(options.target).find(".table-data-summary").replaceWith(
				'<div class="table-data-summary">第 '
						...
						+ ' - ' + (options.currentpage * options.pagesize)
						+ '条纪录 / 共 ' + totalCounter + '条纪录</div>');
		$(options.target).children().find(".pagination-info").replaceWith(
				'<span class="pagination-info">第' + options.currentpage
						+ '页 / 共 ' + paggerCounter + '页 </span>');
		...
		$(options.target).children().find(".pagination-button").each(
				function() {
					...
					} else if (options.currentpage == paggerCounter) {
